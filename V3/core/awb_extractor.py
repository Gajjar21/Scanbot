...
AWB_CONTEXT_KEYWORDS: Tuple[str, ...] = tuple(
    dict.fromkeys(
        list(
            getattr(
                config,
...
def extract_candidates_near_keywords(
    ...
    *line_lookahead* / *line_lookback* control how many lines around a
    ...
    def _has_awb_label(window_text: str) -> bool:
        ...
        return any(
            re.search(p, window_text, flags=re.IGNORECASE) for p in relaxed
        )
    ...
    for i, line in enumerate(lines):
        ...
        has_label = _has_awb_label(line_u) or any(
            kn and kn in line_norm for kn in keywords_norm
        )
        if not has_label:
            continue
        block = " ".join(
