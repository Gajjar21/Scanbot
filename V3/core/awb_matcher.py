...
ALLOW_STANDARD_TOLERANCE = getattr(config, "ALLOW_STANDARD_TOLERANCE", True)
TOLERANCE_HIGH_MAX_DISTANCE = int(getattr(config, "TOLERANCE_HIGH_MAX_DISTANCE", 2))
TOLERANCE_STANDARD_MAX_DISTANCE = int(
    getattr(config, "TOLERANCE_STANDARD_MAX_DISTANCE", 1)
)
...
REQUIRE_SINGLE_STANDARD_CANDIDATE_FOR_TOL = bool(
    getattr(config, "REQUIRE_SINGLE_STANDARD_CANDIDATE_FOR_TOL", True)
)
...
def pick_unique_close_match(
    ...
    if not pool:
        pool = awb_set
    scored = [
        (a, hamming(candidate, a)) for a in pool if hamming(candidate, a) <= max_distance
    ]
    if not scored:
        return None
...
def decide_from_candidates(
    ...
    """Simple decision: exact match first, then optional 1-digit tolerance.
