...
#   - Clear All, Upload Files, Retry Review, Refresh DB
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        def _btn(parent, text, cmd, width=15, bg_color=BTN_BG, **kw):
            b = tk.Button(
                ...
                font=FONT_BTN, relief="raised", bd=2, bg=bg_color, **kw,
            )
            return b

        self.btn_get_awb       = _btn(btn_frame, "▶  Start AWB",        self.on_toggle_get_awb,      width=16)
        self.btn_edm           = _btn(btn_frame, "EDM: DISABLED",       self.on_toggle_edm_checker,  width=16, bg_color="#e8e8e8")
        self.btn_batch         = _btn(btn_frame, "⚙  Prepare Batch",    self.on_prepare_batch,       width=16)
        self.btn_full_cycle    = _btn(btn_frame, "⟳  Full Cycle Once",  self.on_run_full_cycle_once, width=16, bg_color="#eef7ff")
        self.btn_retry_review  = _btn(btn_frame, "↩  Retry Review",     self.on_retry_needs_review,  width=14, bg_color="#fff8e6")
        self.btn_upload        = _btn(btn_frame, "⬆  Upload Files",     self.on_upload_files,        width=14, bg_color="#deeeff")
        self.btn_auto          = _btn(btn_frame, "⚡  AUTO MODE",        self.on_toggle_auto_mode,    width=14, bg_color="#eefce8")
        self.btn_clear_all     = _btn(btn_frame, "🗑  Clear All",        self.on_clear_all,           width=12, bg_color="#fff0f0")
        self.btn_clear_log     = _btn(btn_frame, "Clear Log",            self.clear_log,              width=10)

        for col, b in enumerate([
            self.btn_get_awb, self.btn_edm, self.btn_batch,
        ...
        for col, (label, path) in enumerate(folder_btns):
            tk.Button(
                open_frame, text=label, width=14, font=FONT_SMALL,
                command=lambda p=path: self.open_folder(p),
            ).grid(row=0, column=col, padx=3, pady=2)

        # ── Separator ────────────────────────────────────────────────────────
        tk.Frame(self, height=1, bg="#c0c0c0").pack(fill="x", padx=10, pady=(4, 0))

        # ── Live status strip ────────────────────────────────────────────────
        live_frame = tk.Frame(self, bd=1, relief="groove")
        live_frame.pack(fill="x", padx=10, pady=(4, 0))

        self.lbl_live_awb   = tk.Label(live_frame, text="AWB: OFF",       width=20, anchor="w", font=(*FONT_LABEL[:2], "bold"))
        self.lbl_live_edm   = tk.Label(live_frame, text="EDM: DISABLED",  width=20, anchor="w", font=(*FONT_LABEL[:2], "bold"))
        self.lbl_live_batch = tk.Label(live_frame, text="BATCH: IDLE",    width=20, anchor="w", font=(*FONT_LABEL[:2], "bold"))
        self.lbl_live_auto  = tk.Label(live_frame, text="AUTO: OFF",      width=28, anchor="w", font=(*FONT_LABEL[:2], "bold"))
        ...
        self.lbl_inbox     = tk.Label(counts_frame, text="INBOX: 0",        width=13, anchor="w", font=FONT_SMALL)
        self.lbl_processed = tk.Label(counts_frame, text="PROCESSED: 0",    width=15, anchor="w", font=FONT_SMALL)
        self.lbl_clean     = tk.Label(counts_frame, text="CLEAN: 0",        width=13, anchor="w", font=FONT_SMALL)
        self.lbl_rejected  = tk.Label(counts_frame, text="REJECTED: 0",     width=14, anchor="w", font=FONT_SMALL)
        self.lbl_review    = tk.Label(counts_frame, text="NEEDS_REVIEW: 0", width=18, anchor="w", font=FONT_SMALL)
        self.lbl_out       = tk.Label(counts_frame, text="OUT batches: 0",  width=16, anchor="w", font=FONT_SMALL)
        self.lbl_pending   = tk.Label(counts_frame, text="PENDING: 0",      width=12, anchor="w", font=FONT_SMALL)
        ...
        tk.Label(stats_frame, text="TODAY:", font=(*FONT_SMALL[:2], "bold"), width=6).grid(row=0, column=0, padx=4)
        ...
        for col, (key, text, color) in enumerate(stat_defs, start=1):
            kw = {"font": FONT_SMALL, "padx": 6, "width": 12, "anchor": "w"}
            if color:
                kw["fg"] = color
        ...
        tk.Label(
            bottom_bar, text="V3", font=FONT_SMALL, fg="#999999",
    ...
    def _prompt_employee_number(self):
        ...
        tk.Label(
            dialog, text="AWB Pipeline V3 \u2014 Employee Login",
            font=(*FONT_LABEL[:2], "bold"),
        ).pack(pady=(18, 6))

        tk.Label(
            dialog, text="Enter your Employee Number to begin:",
            font=FONT_LABEL,
        ).pack()

        emp_var = tk.StringVar(value=prev)
        entry = tk.Entry(
            dialog, textvariable=emp_var, font=FONT_LABEL,
        ...
        err_lbl = tk.Label(dialog, text="", fg="red", font=FONT_SMALL)
        err_lbl.pack()
        ...
        tk.Button(
            dialog, text="Login", command=_confirm,
            width=12, font=(*FONT_LABEL[:2], "bold"),
